"""SQLite database bootstrap for SermonScript.
...
logger = logging.getLogger("sermonscript.storage")

DEFAULT_DB_FILENAME = "sermonscript.db"
SCHEMA_VERSION = 2
...
def default_db_path() -> Path:
    """Return ``<user_data_dir>/sermonscript.db``."""
...
def initialise_database(db_path: Path | None = None) -> Path:
    ...
    logger.debug("Initialised SermonScript DB at %s", resolved)
    return resolved
