...
class TranscriptEditorWidget(QWidget):
    ...
    def _on_search(self) -> None:
        ...
        for row in range(0, current_row + 1):
            ...
            if item and contains(item.text()):
                self.segment_table.setCurrentItem(item)
                return
        QMessageBox.information(self, "SermonScript", "검색어를 찾지 못했습니다.")
    ...
    def _on_replace_all(self) -> None:
        ...
        QMessageBox.information(
            self, "SermonScript", f"{changes}개 세그먼트를 치환했습니다."
        )
    ...
    def _on_apply_suggestion(self) -> None:
        ...
        if seg is None:
            QMessageBox.warning(self, "SermonScript", "대상 세그먼트를 찾지 못했습니다.")
            return
