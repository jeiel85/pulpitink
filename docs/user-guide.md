...
본 가이드는 pulpit-ink v0.4.x(예정 v1.0) 의 일반 사용자 워크플로우를 다룹니다.
...
설치 후 어디서든 `pulpit-ink` 명령이 동작합니다.

### 1.3 Portable ZIP 사용 (선택)

Python 설치가 어렵다면 릴리즈 페이지에서 `PulpitInk_Portable_<version>.zip` 을 받아 임의의 폴더에 풀고
`PulpitInk.exe` 를 실행하면 됩니다. FFmpeg 와 STT 모델은 번들에 포함되지 않으므로 별도로 준비해야 합니다.
...
```powershell
pulpit-ink transcribe sermon.mp3
```
...
```powershell
pulpit-ink transcribe sermon.mp3 --reference sermon.md --language ko
```
...
```powershell
pulpit-ink transcribe sermon.mp3 --user-dict .\my-dict.json
```
...
```powershell
pulpit-ink jobs list --limit 20
pulpit-ink jobs show <job-id>
pulpit-ink jobs export <job-id> --format md,srt --output .\re-exports
```
...
```powershell
pulpit-ink settings show
pulpit-ink settings set language ko
pulpit-ink settings set model small
pulpit-ink settings set preset sermon
pulpit-ink settings set output_dir D:\sermons\exports
pulpit-ink settings set model_cache_dir D:\sermons\models
pulpit-ink settings set fuzzy_matching_enabled true
pulpit-ink settings set fuzzy_threshold 0.75
```
...
| 로그 | `%LOCALAPPDATA%\PulpitInk\PulpitInk\Logs\` 또는 platform-log dir |
...
```powershell
pulpit-ink --verbose transcribe sermon.mp3
```
