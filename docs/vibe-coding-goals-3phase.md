# PulpitInk 바이브 코딩 `/goal` 프롬프트 묶음 — 3단계 대형 Goal 버전

이 문서는 PulpitInk를 바이브 코딩 에이전트로 개발할 때 사용할 `/goal` 프롬프트를 **3개의 큰 작업 단위**로 재구성한 버전입니다.
...
```text
/goal
PulpitInk 프로젝트의 초기 저장소 구조, CLI 기반 코어, 오디오 전처리, STT 변환, Export 파이프라인을 한 번에 구현한다.

배경:
PulpitInk는 Windows PC용 오픈소스 설교 녹음 STT 프로그램이다.
...
11. AGENTS.md는 PulpitInk 프로젝트 설정값에 맞게 유지한다.

구현 요구사항 B: doctor 명령
1. PulpitInk doctor 명령을 구현한다.
...
1. PulpitInk transcribe 명령을 추가한다.
2. 명령 형식은 아래를 지원한다.
   PulpitInk transcribe input.mp3 --language ko --model small --preset sermon --output ./exports --format txt,json,md,srt,vtt
3. 입력 파일 존재 여부를 검증한다.
4. 지원 확장자는 다음으로 한다.
   - .mp3
   - .wav
...
- PulpitInk transcribe input.mp3 --language ko --model small 실행 가능
- processed.wav 생성
- TXT export 생성
- JSON export 생성
- Markdown export 생성
...
```text
/goal
PulpitInk의 로컬 작업 이력, 설정 시스템, 모델 관리 기반, PySide6 GUI, 작업 큐와 진행률 표시를 구현한다.
...
1. PulpitInk jobs list 명령을 추가한다.
2. PulpitInk jobs show <job_id> 명령을 추가한다.
3. PulpitInk jobs export <job_id> 명령을 추가한다.
...
5. 설정 항목은 다음을 포함한다.
   ...
   - 기본 출력 폴더
   - 모델 캐시 폴더
6. PulpitInk settings show 명령을 추가한다.
7. PulpitInk settings set <key> <value> 명령을 추가한다.
8. model service 기반을 만든다.
9. PulpitInk models list 명령을 추가한다.
10. PulpitInk models cache-dir 명령을 추가한다.
...
```text
/goal
PulpitInk의 타임스탬프 편집기, 설교 후처리, 사용자 사전, 설교 원문 대조 변환, Windows 릴리즈 패키징 기반을 구현한다.

배경:
Goal 2까지 로컬 오디오 기반 STT, 전처리, Export, SQLite 작업 이력, 설정 시스템, PySide6 GUI, 작업 큐가 동작한다고 가정한다.
이번 Goal은 PulpitInk를 단순 Whisper GUI가 아니라 설교 녹음 전문 STT 워크플로우 앱으로 차별화하는 단계다.
...
20. CLI에도 reference 입력 옵션을 추가한다.
   예:
   PulpitInk transcribe sermon.mp3 --reference sermon.md --language ko --model medium
...
5. 빌드 산출물 이름 규칙:
   - PulpitInk_Portable_{version}.zip
   - PulpitInk_Setup_{version}.exe 는 후속 작업 가능
