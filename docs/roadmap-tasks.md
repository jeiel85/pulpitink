...
`feat/tauri-hybrid` 브랜치는 2026-05-21 기준 현재 `main`의 PulpitInk 코어와 병합되어 다시 빌드 가능한 상태가 되었습니다. 다음 작업은 Tauri GUI를 실제 기본 GUI 후보로 끌어올리기 위한 기능 포팅과 수동 E2E 검증입니다.

1. **[최우선 후보] Tauri GUI 기능 포팅 2차: PySide6 주요 기능 parity 확보**
   - 배치 큐, YouTube URL opt-in 고지/동의 흐름, Glossary 사전 편집, 업데이트 알림을 React/Tauri 화면에 순차 포팅합니다.
   - 기존 Python core/service 계층을 우선 재사용하고, Tauri는 사이드카 호출과 상태 표시를 담당합니다.

2. **[후보 2] Tauri 설치본 수동 E2E 검증**
   - `PulpitInk_0.4.7_x64-setup.exe` 또는 MSI로 설치 후 첫 실행, 온보딩, 시스템 진단, 파일 변환, 검수 에디터, export 경로까지 Windows 10/11 기준으로 확인합니다.
   - 검증 중 발견되는 파일 선택 UX, JSON 파싱, sidecar `_internal` 포함 문제를 우선 수정합니다.

3. **[후보 3] Tauri 파일 선택/폴더 선택 네이티브 다이얼로그 도입**
   - 현재 절대 경로 직접 입력 UX를 Tauri dialog 플러그인 기반 선택 흐름으로 개선합니다.
   - 새 의존성 또는 Tauri 플러그인 추가가 필요한 경우 라이선스와 번들 크기를 기록합니다.
...
4. **[후보 4] 실시간 마이크 녹음 및 오프라인 스트리밍 STT 연동 (GitHub Issue #1 대응)**
...
5. **[후보 5] PyInstaller 배포본 물리 패키징 검증 및 Portable ZIP 검수**
...
6. **[후보 6] GPU 가속 (CUDA/cuDNN) 자동 진단 GUI 및 셋업 가이드**
...
7. **[후보 7] 로컬 오프라인 한국어 맞춤법 사전 및 띄어쓰기 가이드 연동**
...
8. **[후보 8] 사용자 맞춤 용어(Glossary) 교정 기여도 분석 통계 대시보드**
