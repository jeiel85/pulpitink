# SermonScript 3-Goal Vibe Coding Prompts — Fixed
...
```text
/goal
SermonScript의 초기 저장소, CLI 코어, 오디오 전처리, STT, Export 파이프라인을 구현한다.

배경:
SermonScript는 Windows PC용 오픈소스 설교 녹음 STT 프로그램이다. v1.0은 로컬 오디오/비디오 파일만 지원하며 YouTube URL 입력, 온라인 다운로드, 클라우드 기능은 제외한다.
...
2. src/sermonscript 아래 app, cli, core/audio, core/transcription, core/export, core/postprocess, core/reference, services, storage, ui 구조를 만든다.
3. Typer CLI와 sermonscript doctor 명령을 구현한다.
4. doctor는 Python 버전, OS, FFmpeg 설치 여부, 쓰기 권한, 앱 데이터 경로 생성을 확인한다.
5. sermonscript transcribe 명령을 구현한다.
...
13. CLI 예: sermonscript transcribe input.mp3 --language ko --model small --preset sermon --output ./exports --format txt,json
...
- sermonscript transcribe로 TXT/JSON/MD/SRT/VTT 생성 가능
- processed.wav 생성
- pytest 통과
- ruff check 통과
...
```text
/goal
SermonScript의 타임스탬프 편집기, 설교 후처리, 사용자 사전, 원문 대조 변환, Windows 릴리즈 패키징을 구현한다.
...
10. CLI 옵션 예: sermonscript transcribe sermon.mp3 --reference sermon.md --language ko
...
5. 산출물 이름은 SermonScript_Portable_{version}.zip 형식을 사용한다.
