<#
.SYNOPSIS
    Bundles the PyInstaller output into SermonScript_Portable_{version}.zip.

.DESCRIPTION
    Expects dist/SermonScript/ to exist (produced by PyInstaller running
    sermonscript.spec). Resolves the project version from pyproject.toml
...
$BuildDir = Join-Path $DistDir "SermonScript"
...
$ArchiveName = "SermonScript_Portable_$Version.zip"
...
2. STT 모델
   ...
   - 오프라인 환경에서는 sermonscript settings 명령으로 model_cache_dir 을 지정하세요.

3. 실행
   - SermonScript.exe (GUI)
   - sermonscript.exe (CLI) — 별도 wheel 설치 또는 ``python -m sermonscript.cli.main`` 사용
