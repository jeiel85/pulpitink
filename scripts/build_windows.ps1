...
.DESCRIPTION
    ...
    3. Builds the PyInstaller bundle from pulpitink.spec.
...
.PARAMETER Version
    ...
    the project version exposed by ``python -c "import pulpitink"``.
#>
...
$env:PULPITINK_ROOT = "$RepoRoot"
if (Test-Path "build") { Remove-Item "build" -Recurse -Force }
if (Test-Path "dist/PulpitInk") { Remove-Item "dist/PulpitInk" -Recurse -Force }
python -m PyInstaller --clean --noconfirm pulpitink.spec
