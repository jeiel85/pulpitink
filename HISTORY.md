...
- 변경 파일:
  - frontend/package.json, frontend/package-lock.json (`pulpit-ink-app` v0.4.7 메타데이터 반영)
  - frontend/src-tauri/Cargo.toml, Cargo.lock, tauri.conf.json, src/lib.rs, src/main.rs (PulpitInk 앱/사이드카 명칭 및 번들 설정 갱신)
  - frontend/src/App.tsx, frontend/src/App.css (온보딩, 테마 토글, 버튼 크기 및 브랜딩 개선)
  - src/pulpit_ink/cli/main.py (`jobs list/show --json`, `settings show --json` 추가)
  - CHANGELOG.md, HISTORY.md, docs/decision-log.md, docs/roadmap-tasks.md (작업 이력 및 후속 계획 기록)
- 검증:
  - `npm run build` (frontend): PASS
  - `cargo check` (frontend/src-tauri): PASS
  - `npm run tauri build` (frontend): PASS, `PulpitInk_0.4.7_x64-setup.exe` 및 MSI 생성 확인
  - `python -m ruff check src/pulpit_ink/cli/main.py`: PASS
  - `$env:PYTHONPATH="src"; python -m pytest tests/test_settings_service.py tests/test_storage.py tests/test_exporters.py`: 26/26 PASS
- 결과: 성공. Tauri 하이브리드 브랜치가 현재 PulpitInk 코어를 기준으로 다시 빌드 가능한 상태가 되었고, 최초 사용자 안내 UI가 포함되었습니다.
- 후속 작업:
  - Tauri UI에서 PySide6의 배치 큐, YouTube opt-in, Glossary, 업데이트 알림 기능을 동일한 UX 수준으로 순차 포팅
  - Tauri 설치본을 실제 실행해 파일 선택, 변환, 검수, export까지 수동 E2E 확인
