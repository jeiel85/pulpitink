...
v1.0에서 지원하는 핵심 입력은 로컬 파일입니다. 현재 `main`에는 사용자가 저작권/이용 권한 고지에 동의한 뒤 사용하는 YouTube URL 입력도 opt-in 기능으로 포함되어 있습니다.

```text
지원: mp3, wav, m4a, flac, ogg, mp4 등 로컬 파일, 고지 동의 기반 YouTube URL
제외: 일반 온라인 다운로드, 클라우드 입력, 실시간 녹음
```

YouTube URL 입력은 STT core가 아니라 별도 입력 소스 계층으로만 다루며, 저작권/이용 권한 고지와 사용자 확인 절차를 유지해야 합니다.
...
- YouTube 다운로드 범위 확장, 클라우드 연동, 온라인 API 호출 기능 추가
...
- 테스트 구조 전체 변경
- 프로젝트 설정의 대규모 재구성
- YouTube URL 기능의 범위 확장 또는 우회성 다운로드 기능 임의 구현
