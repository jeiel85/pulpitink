...
### Tauri
- `feat/tauri-hybrid` 브랜치를 현재 `main` 기능 위로 병합하여 PulpitInk Python 코어와 React/Tauri 셸을 다시 연결했습니다.
- Tauri 사이드카 명칭과 앱 메타데이터를 `PulpitInk` / `pulpit-ink-sidecar` / `com.jeiel85.pulpitink` 기준으로 정리했습니다.
- Tauri UI에 첫 실행 온보딩 패널, 라이트/다크 테마 전환, 더 큰 기본 버튼 크기, PulpitInk 브랜딩을 반영했습니다.
- Tauri IPC에서 작업 목록과 설정을 안정적으로 읽을 수 있도록 `jobs list --json`, `jobs show --json`, `settings show --json` 출력을 추가했습니다.
